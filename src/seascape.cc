...
ss::V3 get_sea_color(ss::V3 p, ss::V3 n, ss::V3 l, ss::V3 eye, ss::V3 dist)
{
    ...
    const auto reflected = get_sky_color(ss::reflect(eye, n));
...
ss::V3 get_pixel(ss::V2 coord, float time)
{
    ...
    auto dist = p - ori;
    auto n = get_normal(p, ss::dot(dist, dist) * EPSILON_NRM);
    auto light = ss::normalize(ss::V3 { 0.f, 1.f, 0.8f });
    return ss::mix(get_sky_color(dir), get_sea_color(p, n, light, dir, dist), ss::pow(ss::smoothstep(0.f, -0.02f, dir.y), 0.2f));
}

uint32_t shade(const ss::V2 coord)
{
    const auto time = ss::uptime() * 0.3f;
    const auto v = ss::pow(get_pixel(ss::res - coord, time), 0.65f);
    return v.color(1.f);
}
}
